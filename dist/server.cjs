...
const updateUserStatus$1 = async (adminId, userId, payload) => {
	if (adminId === userId && payload.isActive === false) throw new AppError(http_status.default.BAD_REQUEST, "You cannot ban your own admin account");
...
const adminPropertyAvailabilitySchema = zod.default.object({ isAvailable: zod.default.boolean({ error: (issue) => issue.input == null ? "Property availability is required" : "Property availability must be true or false" }) });
//#endregion
...
const updateRentalAgreementStatus$1 = async (tenantId, rentalAgreementId, payload) => {
	...
	if (rentalAgreement.tenantId !== tenantId) throw new AppError(http_status.default.FORBIDDEN, "You cannot update another tenant's rental agreement");
	if (rentalAgreement.status === payload.status) return { status: rentalAgreement.status };
	if (rentalAgreement.status !== RentalAgreementStatus.ACTIVE) throw new AppError(http_status.default.CONFLICT, `Only an active rental agreement can be marked as ${payload.status.toLowerCase()}`);
	if (payload.status === RentalAgreementStatus.COMPLETED && rentalAgreement.leaseEndDate.getTime() > Date.now()) throw new AppError(http_status.default.BAD_REQUEST, "This rental cannot be completed before its lease end date");
	...
	throw new AppError(http_status.default.CONFLICT, "Rental agreement status changed. Refresh and try again.");
};
...
const createReview$1 = async (tenantId, rentalAgreementId, payload) => {
	...
	if (!rentalAgreement) throw new AppError(http_status.default.NOT_FOUND, "Rental agreement not found");
	if (rentalAgreement.tenantId !== tenantId) throw new AppError(http_status.default.FORBIDDEN, "You cannot review another tenant's rental agreement");
	if (!(rentalAgreement.status === RentalAgreementStatus.ACTIVE || rentalAgreement.status === RentalAgreementStatus.COMPLETED)) throw new AppError(http_status.default.BAD_REQUEST, "Only active or completed rentals can be reviewed");
	if (rentalAgreement.payments.length === 0) throw new AppError(http_status.default.BAD_REQUEST, "A successful payment is required before leaving a review");
	if (rentalAgreement.review) throw new AppError(http_status.default.CONFLICT, "Review already submitted for this agreement");
...
const getTenantPaymentBySessionId$1 = async (sessionId, tenantId) => {
	...
	if (!payment) throw new AppError(http_status.default.NOT_FOUND, "Payment session not found");
	return payment;
};
...
const createCheckoutSession$1 = async (tenantId, userEmail, rentalAgreementId) => {
	...
	if (!config_default.app_url) throw new AppError(http_status.default.INTERNAL_SERVER_ERROR, "Frontend application URL is not configured");
	...
	if (payment.stripeSessionId) {
		const existingSession = await stripe$1.checkout.sessions.retrieve(payment.stripeSessionId);
		if (existingSession.status === "open" && existingSession.url) return { checkoutUrl: existingSession.url };
		if (existingSession.status === "complete") throw new AppError(http_status.default.CONFLICT, "Payment was submitted and is being confirmed");
	}
	...
	if (!checkoutSession.url) throw new AppError(http_status.default.INTERNAL_SERVER_ERROR, "Stripe did not return a checkout URL");
