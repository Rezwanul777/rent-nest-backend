...
import * as runtime$1 from "@prisma/client/runtime/client";
...
const globalErrorHandler = (error, _req, res, _next) => {
	...
	if (error instanceof ZodError) return sendResponse(res, {
...
function getPrismaClientClass() {
	return runtime$1.getPrismaClient(config);
}
runtime$1.Extensions.getExtensionContext;
runtime$1.NullTypes.DbNull, runtime$1.NullTypes.JsonNull, runtime$1.NullTypes.AnyNull;
runtime$1.makeStrictEnum({
	ReadUncommitted: "ReadUncommitted",
	ReadCommitted: "ReadCommitted",
	RepeatableRead: "RepeatableRead",
	Serializable: "Serializable"
});
runtime$1.Extensions.defineExtension;
//#endregion
...
const createAccessToken = (payload) => jwt.sign(payload, config_default.jwt_access_secret, { expiresIn: config_default.jwtAccessExpiresIn });
const createRefreshToken = (payload) => jwt.sign(payload, config_default.jwt_refresh_secret, { expiresIn: config_default.jwtRefreshExpiresIn });
const verifyAccessToken = (token) => jwt.verify(token, config_default.jwt_access_secret);
const verifyRefreshToken = (token) => jwt.verify(token, config_default.jwt_refresh_secret);
//#endregion
...
const authenticate = catchAsync(async (req, _res, next) => {
	...
	if (!token) throw new AppError(httpStatus.UNAUTHORIZED, "Please login to continue");
	...
	if (!user.isActive) throw new AppError(httpStatus.FORBIDDEN, "Your account is inactive");
	req.user = user;
	next();
});
//#endregion
...
const register$1 = async (payload) => {
	...
	if (await prisma.user.findUnique({ where: { email } })) throw new AppError(httpStatus.CONFLICT, "User already exists with this email");
...
const login = async (payload) => {
	...
	if (!user) throw new AppError(httpStatus.UNAUTHORIZED, "Invalid credentials", [{ message: "Email or Password didn't match" }]);
	if (!await bcrypt.compare(password, user.password)) throw new AppError(httpStatus.UNAUTHORIZED, "Invalid credentials", [{ message: "Email or Password didn't match" }]);
	if (!user.isActive) throw new AppError(httpStatus.FORBIDDEN, "Account is not active. Please contact support.");
	const tokenPayload = {
		userId: user.id,
		role: user.role
	};
...
const refreshAccessToken = async (payload) => {
	...
	if (!user) throw new AppError(httpStatus.UNAUTHORIZED, "Authentication required. Please login to continue");
	if (!user.isActive) throw new AppError(httpStatus.UNAUTHORIZED, "Account is inactive. Please contact support");
...
const registerSchema = z$1.object({
	name: z$1.string({ error: (issue) => issue.input == null ? "Name is required" : "Name must be a string" }).trim().min(3, "Name must be at least 3 characters").max(255, "Name cannot exceed 255 characters"),
	email: z$1.email({ error: (issue) => issue.input == null ? "Email is required" : "Please provide a valid email" }),
	password: z$1.string("Password is required").min(8, "Password must be at least 8 characters").max(255, "Password cannot exceed 255 characters"),
	role: z$1.enum([UserRole.TENANT, UserRole.LANDLORD], { error: (issue) => issue.input == null ? "Role is required" : "Invalid role! Role can either be TENANT or LANDLORD" })
});
const loginSchema = z$1.object({
	email: z$1.email({ error: (issue) => issue.input == null ? "Email is required" : "Please provide a valid email" }),
	password: z$1.string("Password is required").trim().min(1, "Password is required")
});
//#endregion
...
const authorize = (...roles) => (req, _res, next) => {
	if (!req.user) throw new AppError(httpStatus.UNAUTHORIZED, "Please login to continue");
	if (!roles.includes(req.user.role)) throw new AppError(httpStatus.FORBIDDEN, "You are not allowed to perform this action");
	next();
};
//#endregion
...
const createProperty$1 = async (landlordId, payload) => {
	if (!await prisma.category.findUnique({ where: { id: payload.categoryId } })) throw new AppError(httpStatus.BAD_REQUEST, "Category not found");
...
const getAllUsers$1 = async (query) => {
	...
	if (role) {
		if (!isValidEnumValue(UserRole, role)) throw new AppError(httpStatus.BAD_REQUEST, "Invalid Role");
...
const createCategorySchema = z.object({ name: z.string({ error: (issue) => issue.input == null ? "Category name is required" : "Category name must be a string" }).trim().min(1, "Category name cannot be empty").max(150, "Category name cannot exceed 150 characters") });
//#endregion
...
const createCategory$1 = async (payload) => {
	...
	if (await prisma.category.findFirst({ where: { slug } })) throw new AppError(httpStatus.CONFLICT, "Category already exists");
...
const updateCategory$1 = async (categoryId, payload) => {
	if (!await prisma.category.findUnique({ where: { id: categoryId } })) throw new AppError(httpStatus.NOT_FOUND, "Category not found");
	const slug = createSlug(payload.name);
	if (await prisma.category.findFirst({ where: { slug } })) throw new AppError(httpStatus.CONFLICT, "Category already exists with that name");
...
const createPropertySchema = z.object({
	title: z.string("Title is required").trim().min(5, "Title must be at least 5 characters").max(150, "Title cannot exceed 150 characters"),
	description: z.string("Description is required").trim().min(20, "Description must be at least 20 characters"),
	rent: z.number("Rent is required").positive("Rent must be greater than 0"),
	location: z.string("Location is required").trim().min(2, "Location must be at least 2 characters").max(100, "Location cannot exceed 100 characters"),
	amenities: z.array(z.string().trim()).optional(),
	bedrooms: z.number().int("Bedrooms must be a whole number").positive("Bedrooms must be greater than 0").optional(),
	bathrooms: z.number().int("Bathrooms must be a whole number").positive("Bathrooms must be greater than 0").optional(),
	size: z.number().positive("Size must be greater than 0").optional(),
	images: z.array(z.url("Image must be a valid URL")).optional(),
	categoryId: z.uuid({ error: (issue) => issue.input == null ? "Category is required" : "Invalid category id" })
});
const updatePropertySchema = z.object({
	title: z.string().trim().min(5, "Title must be at least 5 characters").max(200, "Title cannot exceed 200 characters").optional(),
	...
	location: z.string().trim().min(2, "Location must be at least 2 characters").max(100, "Location cannot exceed 100 characters").optional(),
	amenities: z.array(z.string().trim()).optional(),
	bedrooms: z.number().int("Bedrooms must be a whole number").positive("Bedrooms must be greater than 0").optional(),
	bathrooms: z.number().int("Bathrooms must be a whole number").positive("Bathrooms must be greater than 0").optional(),
	size: z.number().positive("Size must be greater than 0").optional(),
...
z.object({ status: z.enum([RentalRequestStatus.APPROVED, RentalRequestStatus.REJECTED], { error: (issue) => issue.input == null ? "Status is required" : "Invalid status! Status can either be APPROVED or REJECTED" }) });
...
const buildRentalRequestFilters = (query, scope) => {
	...
	if (query.status) {
		if (!isValidEnumValue(RentalRequestStatus, query.status)) throw new AppError(httpStatus.BAD_REQUEST, "Invalid rental request status");
...
const updateRentalRequestStatus$1 = async (landlordId, requestId, payload) => {
	...
	if (!rentalRequest) throw new AppError(httpStatus.NOT_FOUND, "Rental request not found");
	if (rentalRequest.status !== RentalRequestStatus.PENDING) throw new AppError(httpStatus.BAD_REQUEST, "Only pending rental requests can be updated");
...
const submitRentalRequestSchema = z$1.object({
	propertyId: z$1.uuid({ error: (issue) => issue.input == null ? "Property ID is required" : "Invalid property ID" }),
	tenantMessage: z$1.string().trim().min(3, "Message must be at least 3 characters").optional(),
	requestedMoveInDate: z$1.coerce.date({ error: (issue) => issue.input == null ? "Date is requried" : "Invalid date" }),
	durationInMonths: z$1.coerce.number({ error: (issue) => issue.input == null ? "Duration in months is required" : "Invalid duration" }).int("Duration must be a whole number").min(1, "Duration must be minimum 1 months").max(36, "Duration cannot exceed 36 months")
});
const updateRentalRequestStatusSchema = z$1.object({ status: z$1.enum([RentalRequestStatus.APPROVED, RentalRequestStatus.REJECTED], { error: (issue) => issue.input == null ? "Status is required" : "Invalid status! Status can either be APPROVED or REJECTED" }) });
//#endregion
...
const buildRentalAgreementFilters = (query, scope) => {
	...
	if (query.status) {
		if (!isValidEnumValue(RentalAgreementStatus, query.status)) throw new AppError(httpStatus.BAD_REQUEST, "Invalid status query");
		andCondition.push({ status: query.status });
	}
	return andCondition;
};
...
const updateRentalAgreementStatusSchema = z.object({ status: z.enum([RentalAgreementStatus.TERMINATED, RentalAgreementStatus.COMPLETED], { error: (issue) => issue.input == null ? "Status is required" : "Invalid status! Status can either be TERMINATED or COMPLETED" }) });
//#endregion
...
const createReviewSchema = z.object({
	rating: z.number("Rating is required").int("Rating should be a whole number").min(1, "Rating should be at least 1").max(5, "Rating can be maximum 5"),
...
const handleStripeWebhook$1 = async (payload, signature) => {
	...
	try {
		...
		throw new AppError(httpStatus.BAD_REQUEST, "⚠️ Webhook signature verification failed.");
	}
...
const createCheckoutSession$1 = async (tenantId, userEmail, rentalAgreementId) => {
	...
	if (rentalAgreement.status !== RentalAgreementStatus.PENDING_PAYMENT) throw new AppError(httpStatus.BAD_REQUEST, "Payment is not available for this agreement");
	if (!config_default.app_url) throw new AppError(httpStatus.INTERNAL_SERVER_ERROR, "Frontend application URL is not configured");
	...
	if (payment.stripeSessionId) {
		const existingSession = await stripe.checkout.sessions.retrieve(payment.stripeSessionId);
		if (existingSession.status === "open" && existingSession.url) return { checkoutUrl: existingSession.url };
		if (existingSession.status === "complete") throw new AppError(httpStatus.CONFLICT, "Payment was submitted and is being confirmed");
	}
	...
	if (!checkoutSession.url) throw new AppError(httpStatus.INTERNAL_SERVER_ERROR, "Stripe did not return a checkout URL");
...
const handleCheckoutCompleted = async (tx, session) => {
	...
	if (!paymentId) throw new AppError(httpStatus.BAD_REQUEST, "Payment ID missing from Stripe metadata");
